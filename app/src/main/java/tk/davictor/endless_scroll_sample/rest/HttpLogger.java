 ...
 */
final class HttpLogger implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean pretty = level == Level.BODY_PRETTY;
        boolean logBody = level == Level.BODY || pretty;
    ...
    private void printPrettyBody(String body) {
        try {
            if (body.startsWith("{")) {
                String[] lines = new JSONObject(body).toString(4).split("\\n");
                for (String line : lines) {
                    logger.log(line);
                }
            } else if (body.startsWith("[")) {
                String[] lines = new JSONArray(body).toString(4).split("\\n");
                for (String line : lines) {
                    logger.log(line);
                }
            } else {
